...
// ========== RAG COMPONENTS: VECTOR STORE & EMBEDDINGS ==========
// Vector store for POI embeddings (in-memory)
let vectorStore = null;
let embeddingsCache = new Map(); // Cache embeddings to avoid re-computing
...
// Split POI data into semantic chunks for better retrieval
function chunkPOIData(poi) {
  const chunks = [];
  ...
  chunks.push({
    ...
    poiId: poi.id,
    type: 'basic_info',
    text: `${poi.name}. Категория: ${poi.category}, ${poi.subcategory}. Адрес: ${poi.address || 'не указан'}`,
    metadata: {
      name: poi.name,
  ...
  // Chunk 2: Location and access info
  if (poi.address || poi.district) {
    chunks.push({
      id: `${poi.id}_location`,
      poiId: poi.id,
      type: 'location',
      text: `${poi.name} находится по адресу: ${poi.address || 'не указан'}. Район: ${poi.district || 'не указан'}, город ${poi.city || 'Астана'}`,
      metadata: {
        name: poi.name,
        address: poi.address,
  ...
  // Chunk 3: Working hours and contacts
  if (poi.workingHours || poi.phone || poi.instagram) {
    chunks.push({
      id: `${poi.id}_contacts`,
      poiId: poi.id,
      type: 'contacts',
      text: `${poi.name}. Режим работы: ${poi.workingHours || 'не указан'}. Телефон: ${poi.phone || 'нет'}. Instagram: ${poi.instagram || 'нет'}`,
      metadata: {
        name: poi.name,
...
// ========== RAG: GENERATE EMBEDDINGS USING GEMINI ==========
async function generateEmbedding(text) {
  try {
    ...
    if (!process.env.GEMINI_API_KEY) {
      console.warn('⚠️ GEMINI_API_KEY not set, embeddings disabled');
      return null;
    }
...
async function buildVectorStore(pois, forceRebuild = false) {
  if (vectorStore && !forceRebuild) {
    console.log('📦 Using cached vector store');
    return vectorStore;
  }
  
  console.log('🔨 Building vector store with embeddings...');
  const startTime = Date.now();
  
  const store = {
    chunks: [],
    embeddings: [],
    index: new Map() // poiId -> chunk indices
  };
  
  // Process POIs in batches to avoid rate limits
  const BATCH_SIZE = 10;
  const MAX_POIS = 50; // Limit for demo (можно увеличить)
  const poisToProcess = pois.slice(0, MAX_POIS);
  
  for (let i = 0; i < poisToProcess.length; i += BATCH_SIZE) {
    const batch = poisToProcess.slice(i, i + BATCH_SIZE);
    
    for (const poi of batch) {
      const chunks = chunkPOIData(poi);
      ...
      for (const chunk of chunks) {
        ...
        if (embedding) {
          const chunkIndex = store.chunks.length;
          store.chunks.push(chunk);
          store.embeddings.push(embedding);
          
          // Index by POI ID
          if (!store.index.has(chunk.poiId)) {
            store.index.set(chunk.poiId, []);
          }
          store.index.get(chunk.poiId).push(chunkIndex);
        }
      }
    }
    
    // Small delay to respect rate limits
    if (i + BATCH_SIZE < poisToProcess.length) {
      await new Promise(resolve => setTimeout(resolve, 100));
    }
    
    console.log(`📊 Processed ${Math.min(i + BATCH_SIZE, poisToProcess.length)}/${poisToProcess.length} POIs`);
  }
  
  vectorStore = store;
  const duration = ((Date.now() - startTime) / 1000).toFixed(2);
  console.log(`✅ Vector store built: ${store.chunks.length} chunks, ${store.embeddings.length} embeddings in ${duration}s`);
  
  return vectorStore;
}

// ========== RAG: SEMANTIC SEARCH ==========
async function semanticSearch(query, topK = 10, scoreThreshold = 0.3) {
  if (!vectorStore || vectorStore.chunks.length === 0) {
    console.warn('⚠️ Vector store not initialized, using fallback');
    return [];
  }
  
  console.log(`🔍 Semantic search for: "${query}" (top-${topK}, threshold: ${scoreThreshold})`);
  
  // Generate query embedding
  const queryEmbedding = await generateEmbedding(query);
  if (!queryEmbedding) {
    console.warn('⚠️ Could not generate query embedding');
    return [];
  }
  ...
  for (let i = 0; i < vectorStore.embeddings.length; i++) {
    const score = cosineSimilarity(queryEmbedding, vectorStore.embeddings[i]);
    
    if (score >= scoreThreshold) {
      results.push({
        chunk: vectorStore.chunks[i],
        score: score,
        index: i
      });
    }
...
// ========== RAG: RETRIEVE POIS FROM CHUNKS ==========
function retrievePOIsFromChunks(searchResults, allPOIs) {
  const poiIds = new Set();
  const poiScores = new Map();
  const retrievedChunks = [];
  
  // Collect unique POI IDs and aggregate scores
  for (const result of searchResults) {
    const poiId = result.chunk.poiId;
    poiIds.add(poiId);
    
    // Aggregate scores (max score for POI)
    if (!poiScores.has(poiId) || result.score > poiScores.get(poiId)) {
      poiScores.set(poiId, result.score);
    }
    ...
    retrievedChunks.push({
      ...
      score: result.score,
      poiId: poiId,
      poiName: result.chunk.metadata.name
    });
  }
  ...
  for (const poiId of poiIds) {
    ...
    if (poi) {
      retrievedPOIs.push({
        ...poi,
        relevanceScore: poiScores.get(poiId),
        vectorScore: poiScores.get(poiId) // For transparency
      });
    }
  }
  
  // Sort by relevance
  retrievedPOIs.sort((a, b) => b.relevanceScore - a.relevanceScore);
  
  return { pois: retrievedPOIs, chunks: retrievedChunks };
}
...
// Main recommendation endpoint (RAG-BASED WITH VECTOR SEARCH)
app.post('/api/recommend', async (req, res) => {
  try {
    const { query, location, filters, sessionId, useVectorSearch = true } = req.body;
    ...
    if (useVectorSearch && process.env.GEMINI_API_KEY) {
      try {
        ...
        if (!vectorStore) {
          console.log('🔨 First-time vector store initialization...');
...
ОТВЕТЬ СЕЙЧАС:`;
    ...
    // ========== RAG TRANSPARENCY: Include retrieved context ==========
    const ragMetadata = {
      searchMethod: searchMethod, // 'vector' or 'keyword'
      retrievedChunksCount: retrievedChunks.length,
      retrievedChunks: retrievedChunks.slice(0, 10).map(chunk => ({
        text: chunk.text,
        type: chunk.type,
        score: chunk.score ? chunk.score.toFixed(3) : null,
        poiName: chunk.poiName
      })),
      candidatePOIsCount: relevantPOIs.length,
      topCandidates: relevantPOIs.slice(0, 10).map(poi => ({
        name: poi.name,
        category: poi.category,
        relevanceScore: poi.relevanceScore ? poi.relevanceScore.toFixed(2) : null,
        vectorScore: poi.vectorScore ? poi.vectorScore.toFixed(3) : null,
        distance: poi.distance ? poi.distance.toFixed(2) : null
      })),
      hasInsufficientData: relevantPOIs.length === 0,
      dataQualityCheck: {
        totalPOIsInDatabase: allPOIs.length,
        poisAfterFiltering: relevantPOIs.length,
        filtersApplied: {
          category: selectedCategory !== 'all' ? selectedCategory : null,
          openOnly: showOpenOnly,
          accessibility: needsAccessibility,
          groupSize: groupSize > 1 ? groupSize : null,
          radius: searchRadius
        }
      }
    };
    ...
    if (relevantPOIs.length === 0) {
      console.warn('⚠️ INSUFFICIENT DATA: No POIs found matching criteria');
      return res.json({
        success: true,
        query,
        response: language === 'ru' 
          ? `К сожалению, в базе данных не найдено мест, соответствующих вашему запросу "${query}". Попробуйте изменить фильтры или сформулировать запрос иначе.`
          : language === 'en'
          ? `Sorry, no places found in the database matching your query "${query}". Try changing filters or rephrasing your request.`
          : `Кешіріңіз, "${query}" сұранысына сәйкес орындар табылмады. Сүзгілерді өзгертіп немесе сұранысты басқаша жазып көріңіз.`,
        pois: [],
        poisCount: 0,
        ragMetadata,
    ...
    res.json({
      ...
      ragMetadata: ragMetadata, // NEW: RAG transparency data
      sessionId: userSessionId,
      timestamp: new Date().toISOString()
    });
...
// Initialize vector store endpoint
app.post('/api/rag/init-vector-store', async (req, res) => {
  try {
    console.log('🔨 Initializing vector store via API...');
    const allPOIs = loadPOIData();
    const { forceRebuild = false, maxPOIs = 500 } = req.body;
    
    const store = await buildVectorStore(allPOIs.slice(0, maxPOIs), forceRebuild);
    
    res.json({
      success: true,
      message: 'Vector store initialized successfully',
      chunksCount: store.chunks.length,
      embeddingsCount: store.embeddings.length,
      poisProcessed: maxPOIs,
      timestamp: new Date().toISOString()
    });
  } catch (error) {
...
app.get('/api/rag/status', (req, res) => {
  res.json({
    ...
    chunksCount: vectorStore ? vectorStore.chunks.length : 0,
    embeddingsCount: vectorStore ? vectorStore.embeddings.length : 0,
    embeddingsCacheSize: embeddingsCache.size,
    conversationSessionsCount: conversationMemory.size,
    geocodingCacheSize: geocodingCache.size,
    totalPOIsLoaded: allPOIsCache ? allPOIsCache.length : 0,
    timestamp: new Date().toISOString()
  });
});
...
app.post('/api/rag/test-search', async (req, res) => {
  try {
    const { query, topK = 5, scoreThreshold = 0.3 } = req.body;
    ...
    if (!vectorStore) {
      return res.status(400).json({ 
        ...
        hint: 'Call POST /api/rag/init-vector-store first'
      });
    }
    
    const results = await semanticSearch(query, topK, scoreThreshold);
...
app.get('/api/health', (req, res) => {
  res.json({ 
    status: 'ok',
    ragEnabled: !!process.env.GEMINI_API_KEY,
