...
interface Message {
  ...
  ragMetadata?: {
    ...
    dataQualityCheck?: {
      totalPOIsInDatabase: number;
      poisAfterFiltering: number;
      filtersApplied: any;
    };
  };
}
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const assistantMessage: Message = {
        ...
        timestamp: new Date(),
        ragMetadata: data.ragMetadata // Store RAG transparency data
      };
                      ...
                      <div className={`flex items-start space-x-2 sm:space-x-3 max-w-3xl ${message.type === 'user' ? 'flex-row-reverse space-x-reverse' : ''}`}>
                        ...
                        <div className={`flex-1 ${message.type === 'user' ? 'items-end' : 'items-start'}`}>
                          <div className={`rounded-2xl px-3 py-2 sm:px-4 sm:py-3 ${
                            ...
                            {/* RAG Transparency Info */}
                            {message.type === 'assistant' && message.ragMetadata && (
                              <div className="mt-3 pt-3 border-t border-slate-200">
                                <details className="text-xs text-slate-600">
                                  <summary className="cursor-pointer font-medium text-slate-700 hover:text-blue-600 flex items-center gap-1">
                                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                                    </svg>
                                    {language === 'ru' ? '🔍 Источники и контекст RAG' : language === 'en' ? '🔍 RAG Sources & Context' : '🔍 RAG дереккөздер'}
                                  </summary>
                                  <div className="mt-2 space-y-2 pl-5">
                                    <div className="flex items-center gap-2">
                                      <span className={`px-2 py-0.5 rounded text-xs font-medium ${
                                        message.ragMetadata.searchMethod === 'vector' 
                                          ? 'bg-purple-100 text-purple-700' 
                                          : 'bg-blue-100 text-blue-700'
                                      }`}>
                                        {message.ragMetadata.searchMethod === 'vector' ? '🧬 Vector Search' : '🔤 Keyword Search'}
                                      </span>
                                      <span className="text-slate-500">
                                        {message.ragMetadata.candidatePOIsCount} {language === 'ru' ? 'кандидатов' : 'candidates'}
                                      </span>
                                    </div>
                                    
                                    {message.ragMetadata.retrievedChunks && message.ragMetadata.retrievedChunks.length > 0 && (
                                      <div className="bg-white rounded p-2 border border-slate-200">
                                        <p className="font-medium text-slate-700 mb-1">
                                          {language === 'ru' ? 'Извлечённые фрагменты' : 'Retrieved Chunks'} ({message.ragMetadata.retrievedChunksCount}):
                                        </p>
                                        <ul className="space-y-1 text-xs">
                                          {message.ragMetadata.retrievedChunks.slice(0, 3).map((chunk, idx) => (
                                            <li key={idx} className="text-slate-600">
                                              • <strong>{chunk.poiName}</strong> ({chunk.type}) 
                                              {chunk.score && <span className="text-purple-600 ml-1">score: {chunk.score}</span>}
                                              <br />
                                              <span className="text-slate-500 italic ml-2">{chunk.text.substring(0, 80)}...</span>
                                            </li>
                                          ))}
                                        </ul>
                                      </div>
                                    ...
                                    {message.ragMetadata.dataQualityCheck && (
                                      <div className="text-xs text-slate-500">
                                        📊 База: {message.ragMetadata.dataQualityCheck.totalPOIsInDatabase} POI → 
                                        после фильтров: {message.ragMetadata.dataQualityCheck.poisAfterFiltering}
                                      </div>
                                    )}
                                    
                                    {message.ragMetadata.hasInsufficientData && (
                                      <div className="bg-yellow-50 border border-yellow-200 rounded p-2 text-xs text-yellow-800">
                                        ⚠️ {language === 'ru' ? 'Недостаточно данных в базе для ответа' : 'Insufficient data in database'}
                                      </div>
                                    )}
                                  </div>
                                </details>
